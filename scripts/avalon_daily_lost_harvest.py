...
def summarize_by_day(rows, high_irr_threshold):
    grouped = defaultdict(list)
    for row in rows:
        grouped[utc_day(row[0])].append(row)
...
def main():
    p = argparse.ArgumentParser(description='Daily lost-harvest report from persisted Avalon dry-run data.')
    ...
    report = {
        ...
        'days': summarize_by_day(rows, args.high_irradiance_threshold),
    }
